    ...
    return (
        <ul className='list'>
            ...
            {films.length === 0 ? <h1>movie not found</h1> :
                films.map((el) => (<ItemsFilm title={el.title} img={el.poster_path} overview={el.overview} date={el.release_date} filmsId={el.id} genre={el.genre_ids} filmsGenre={filmsGenre} rating ={el.rating} guestSession={guestSession} key={uuidv4()} />))}
        </ul>
    )
}
